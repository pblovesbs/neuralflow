...
async def _execute_scheduled_workflow(workflow_id: str):
    ...
    try:
        ...
        if not os.path.exists(wf_path):
            await broadcast_log(manager, "system", f"Scheduled workflow {workflow_id} not found on disk.", "ERROR")
            return
        ...
        await broadcast_log(manager, "system", f"⏰ Triggering scheduled workflow: {workflow_id}", "INFO")
...
def add_job(workflow_id: str, cron_expr: Optional[str] = None, interval_seconds: Optional[int] = None):
