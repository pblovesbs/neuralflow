...
async def _generate_embeddings(texts: list[str]) -> list[list[float]]:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        response = await client.post(
            url, json={"model": "nomic-embed-text", "input": texts}
        )
...
async def ingest(
    text: str, collection_name: str = "default", chunk_size: int = 512
) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Embedding failed. Make sure to run 'ollama pull nomic-embed-text'. Details: {e}",
            }
...
async def query(prompt: str, collection_name: str = "default", top_k: int = 3) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Collection '{collection_name}' not found. Store some data first.",
            }
