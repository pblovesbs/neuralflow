...
export default function LandingPage() {
  ...
  return (
    <main className="flex flex-col h-screen w-screen items-center justify-center bg-black overflow-hidden relative">
      ...
      >
        <div className="relative">
          <img 
            src="/logo.png" 
            alt="NeuralFlow Logo" 
            className="w-48 h-48 object-contain animate-hover-throb rounded-3xl"
          />
        </div>
      </div>
      ...
      {/* --- SELECTION PHASE --- */}
      <div 
        className={`w-full h-full flex flex-col justify-between items-center transition-all duration-1000 ease-out ${
      ...
      >
        ...
        <div 
          className={`relative z-10 w-full max-w-md bg-neutral-950/80 backdrop-blur-3xl border border-white/10 rounded-3xl p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${isLaunching ? 'scale-90 opacity-0' : 'scale-100 opacity-100'}`}
        >
          {/* Segmented Control */}
        ...
        <div className={`mb-16 flex flex-col items-center justify-center z-10 transition-opacity duration-700 ${isLaunching ? 'opacity-0' : 'opacity-100'}`}>
          <div className="mb-4">
            <img 
              src="/logo.png" 
              alt="NeuralFlow Logo" 
              className="w-16 h-16 object-contain rounded-2xl shadow-[0_0_30px_rgba(34,211,238,0.3)] transition-transform hover:scale-105"
            />
          </div>
          <h1 
            className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-neutral-200 to-neutral-400 tracking-tighter uppercase drop-shadow-2xl"
